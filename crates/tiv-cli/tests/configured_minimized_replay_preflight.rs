...
#[test]
fn minimized_replay_rejects_missing_source_before_config_or_stack_access() {
    let root = std::env::temp_dir().join(format!(
        "tiv-missing-minimized-replay-source-{}",
        std::process::id()
    ));
    ...
    assert!(stderr.contains("configured minimized replay source artifact is invalid"));
